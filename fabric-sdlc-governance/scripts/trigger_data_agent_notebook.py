...
def _update_notebook_definition(ws_id: str, nb_id: str, ipynb_path: pathlib.Path) -> None:
    ...
    r = requests.post(f"{FABRIC}/workspaces/{ws_id}/items/{nb_id}/updateDefinition",
                      headers=headers(), data=json.dumps(body))
    if r.status_code in (200, 201):
        print("  updated notebook definition")
        return
    if r.status_code == 202:
        ...
        for _ in range(60):
            ...
            if rr.status_code == 200 and rr.json().get("status") == "Succeeded":
                print("  updated notebook definition (async)")
                return
        sys.exit("updateDefinition timed out")
